...
void residualForChemo(FEValues<dim>& fe_values, unsigned int DOF, FEFaceValues<dim>& fe_face_values, const typename DoFHandler<dim>::active_cell_iterator &cell, double dt, dealii::Table<1, Sacado::Fad::DFad<double> >& ULocal, dealii::Table<1, double>& ULocalConv, dealii::Table<1, double>& ULocalConvConv, dealii::Table<1, double>& Pr_ULocalConv,dealii::Table<1, double>& Pr_ULocalConvConv, dealii::Table<1, double>& T_ULocalConv,dealii::Table<1, double>& T_ULocalConvConv, dealii::Table<1, Sacado::Fad::DFad<double> >& R, double currentTime, double totalTime) {
  ...
  for (unsigned int q=0; q<n_q_points; ++q) {  
    ...
    for (unsigned int i=0; i<dofs_per_cell; ++i) {
      ...
      if (ck>=3 && ck<6) {
	vel[q][ck-dim]+=fe_values.shape_value_component(i, q, ck)*ULocal[i];
	vel_conv[q][ck-dim]+=fe_values.shape_value_component(i, q, ck)*ULocalConv[i];
	vel_conv_conv[q][ck-dim]+=fe_values.shape_value_component(i, q, ck)*ULocalConvConv[i];     	
	for (unsigned int j=0; j<dim; j++) {
	  vel_j[q][ck-dim][j]+=fe_values.shape_grad_component(i, q, ck)[j]*ULocal[i];
	  vel_conv_j[q][ck-dim][j]+=fe_values.shape_grad_component(i, q, ck)[j]*ULocalConv[i];
	  vel_conv_conv_j[q][ck-dim][j]+=fe_values.shape_grad_component(i, q, ck)[j]*ULocalConv[i];
	}	
      }
      else if (ck==6) {
  ...
  for (unsigned int f=0; f < faces_per_cell; f++) { 
	...
	for (unsigned int i=0; i<dofs_per_cell; ++i) {
	  ...
	  else if (ck==8) { LiqfaceConv[q]+=fe_face_values.shape_value_component(i, q,ck)*T_ULocalConv[i];}
  ...
  for (unsigned int i=0; i<dofs_per_cell; ++i) {
	...
	//Massterm
	R[i]+=(0.5/dt)*fe_values.shape_value_component(i, q, ck)*(3.0*vel[q][ck-dim]-4.0*vel_conv[q][ck-dim]+vel_conv_conv[q][ck-dim])*fe_values.JxW(q);
	
	//viscous term
	  for (unsigned int j = 0; j < dim; j++){
	    R[i]+= (mmu/RHO)*fe_values.shape_grad_component(i, q, ck)[j]*(vel_j[q][ck-dim][j])*fe_values.JxW(q);
	 ...
	 // pressure (kineitc)
	R[i]+=-(1.0)*fe_values.shape_grad_component(i, q, ck)[ck-dim]*(press_conv[q])*fe_values.JxW(q);
	R[i]+=-(4.0/3.0)*fe_values.shape_grad_component(i, q, ck)[ck-dim]*(phi_conv[q])*fe_values.JxW(q);
	R[i]+=-(-1.0/3.0)*fe_values.shape_grad_component(i, q, ck)[ck-dim]*(phi_conv_conv[q])*fe_values.JxW(q);
	...
	//free convection
	if (ck==4) R[i]+=-fe_values.shape_value_component(i, q, ck)*(gravity*BETA*(T_conv[q]-TSS))*fe_values.JxW(q);
	...
	R[i]+=fe_values.shape_value_component(i, q, ck)*((AA)*vel[q][ck-dim])*fe_values.JxW(q);
	
	
	//advection term
	//first part	
	for (unsigned int j = 0; j < dim; j++){	  
	  R[i]+=fe_values.shape_value_component(i, q, ck)*(vel_star[q][j]*vel_j[q][ck-dim][j])*fe_values.JxW(q);
	}
	//second part
	R[i]+=0.5*fe_values.shape_value_component(i, q, ck)*(vel_star_j[q][0][0]*vel[q][ck-dim])*fe_values.JxW(q);
	R[i]+=0.5*fe_values.shape_value_component(i, q, ck)*(vel_star_j[q][1][1]*vel[q][ck-dim])*fe_values.JxW(q);  
	R[i]+=0.5*fe_values.shape_value_component(i, q, ck)*(vel_star_j[q][2][2]*vel[q][ck-dim])*fe_values.JxW(q);  
  ...
  for (unsigned int f=0; f < faces_per_cell; f++) { 
	...
	for (unsigned int i=0; i<dofs_per_cell; ++i) {
	  ...
	  for (unsigned int q=0; q<n_q_points_face; ++q) {
	    if (ck==3 || ck==5 ) {  
	      R[i] +=-(1.0/RHO)*fe_face_values.shape_value_component(i, q, ck)*(dGammadT*Tfaceconv_j[q][ck-dim])*fe_face_values.JxW(q);  
	    }
