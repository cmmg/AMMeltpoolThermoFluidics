...
void residualForChemo(FEValues<dim>& fe_values, unsigned int DOF, FEFaceValues<dim>& fe_face_values, const typename DoFHandler<dim>::active_cell_iterator &cell, double dt, dealii::Table<1, Sacado::Fad::DFad<double> >& ULocal, dealii::Table<1, double>& ULocalConv, dealii::Table<1, double>& ULocalConvConv, dealii::Table<1, double>& Pr_ULocalConv,dealii::Table<1, double>& Pr_ULocalConvConv, dealii::Table<1, Sacado::Fad::DFad<double> >& R, double currentTime, double totalTime) {
  ...
  dealii::Table<2,Sacado::Fad::DFad<double> > Tface_j(n_q_points,dim);
  ...
  for (unsigned int q=0; q<n_q_points; ++q) {  
    ...
    T[q]=0; liquid[q]=0; liquid_conv[q]=0;
	...
	T[q]+=fe_values.shape_value_component(i, q, ck)*ULocal[i];
     }
    
      else if (ck==4) {
	liquid[q]+=fe_values.shape_value_component(i, q, ck)*ULocal[i];
	liquid_conv[q]+=fe_values.shape_value_component(i, q, ck)*ULocalConv[i];
      }
  ...
  for (unsigned int f=0; f < faces_per_cell; f++) { 
	...
	LiqfaceConv[q]=0;
	for (unsigned int j=0; j<dim ; ++j) { Tface_j[q][j]=0; }
	for (unsigned int i=0; i<dofs_per_cell; ++i) {
	  const unsigned int ck = fe_values.get_fe().system_to_component_index(i).first - DOF;
	  if (ck==3) {
	    for (unsigned int j=0; j<dim ; ++j) {
	      Tface_j[q][j]+=fe_face_values.shape_grad_component(i, q,ck)[j]*ULocal[i];
	    }	    
	  }
	  else if (ck==4) { LiqfaceConv[q]+=fe_face_values.shape_value_component(i, q,ck)*ULocalConv[i];}
  ...
  for (unsigned int i=0; i<dofs_per_cell; ++i) {
    ...
    for (unsigned int q=0; q<n_q_points; ++q) {        
      if(ck>=0 && ck<2) {
	if (liquid_conv[q]>=1.0) {RHO_T=RHOL;}
	else {RHO_T=RHO;}
	...
	for (unsigned int j = 0; j < dim; j++){
	 ...
	 // pressure
	R[i]+=-(1.0/RHO_T)*(1.0)*fe_values.shape_grad_component(i, q, ck)[ck]*(press_conv[q])*fe_values.JxW(q);
	R[i]+=-(1.0/RHO_T)*(4.0/3.0)*fe_values.shape_grad_component(i, q, ck)[ck]*(phi_conv[q])*fe_values.JxW(q);
	R[i]+=-(1.0/RHO_T)*(-1.0/3.0)*fe_values.shape_grad_component(i, q, ck)[ck]*(phi_conv_conv[q])*fe_values.JxW(q);
	

	//free convection
	if (liquid_conv[q]>=1.0)R[i]+=-fe_values.shape_value_component(i, q, ck)*(gravity*BETA*(T[q]-TLL))*fe_values.JxW(q);
	

	//pressure drop due to mush zone
	Sacado::Fad::DFad<double> AA=(1.e+15)*((1.0-liquid[q])*(1.0-liquid[q]))/(liquid[q]*liquid[q]*liquid[q]+1.0e-03);
	if (liquid_conv[q]<1.0) R[i]+=fe_values.shape_value_component(i, q, ck)*(std::abs(AA.val())*vel[q][ck])*fe_values.JxW(q);
