...
template <int dim>
void residualForProjection(FEValues<dim>& fe_values, unsigned int DOF, FEFaceValues<dim>& fe_face_values, const typename DoFHandler<dim>::active_cell_iterator &cell, double dt, dealii::Table<1, double >& ULocal, dealii::Table<1, Sacado::Fad::DFad<double> >& Pr_ULocal, dealii::Table<1, double>& ULocalConv, dealii::Table<1, double>& Pr_ULocalConv, dealii::Table<1, Sacado::Fad::DFad<double> >& Rp, double currentTime, double totalTime) {
  ...
  for (unsigned int q=0; q<n_q_points; ++q) {    
	...
	for (unsigned int j=0; j<dim; j++) {
	  ...
	  //phi_j[q][j]+=fe_values.shape_grad_component(i, q, ck)[j]*Pr_ULocal[i];
	  press_j[q][j]+=fe_values.shape_grad_component(i, q, ck)[j]*Pr_ULocal[i];
	  press_conv_j[q][j]+=fe_values.shape_grad_component(i, q, ck)[j]*Pr_ULocalConv[i];
	}	
      }
  ...
  for (unsigned int i=0; i<dofs_per_cell; ++i) {
	...
	for (unsigned int j=0; j<dim; j++) {
	  ...
	  Rp[i]+=-(1.0)*fe_values.shape_grad_component(i, q, ck)[j]*(press_j[q][j])*fe_values.JxW(q);
	  // Rp[i]+=(1.0)*fe_values.shape_grad_component(i, q, ck)[j]*(press_conv_j[q][j])*fe_values.JxW(q);
