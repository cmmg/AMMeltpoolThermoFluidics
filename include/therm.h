...
void residualForTherm(FEValues<dim>& fe_values, unsigned int DOF, FEFaceValues<dim>& fe_face_values, const typename DoFHandler<dim>::active_cell_iterator &cell, double dt, dealii::Table<1, Sacado::Fad::DFad<double> >& T_ULocal, dealii::Table<1, double>& T_ULocalConv,dealii::Table<1, double>& T_ULocalConvConv, dealii::Table<1, Sacado::Fad::DFad<double> >& ULocal, dealii::Table<1, double>& ULocalConv,dealii::Table<1, double>& ULocalConvConv, dealii::Table<1, Sacado::Fad::DFad<double> >& R, double currentTime, double totalTime) {
  ...
  for (unsigned int i=0; i<dofs_per_cell; ++i) {
	...
	for (unsigned int j = 0; j < dim; j++){
	  R[i] +=(TCONST/STEno)*fe_values.shape_value_component(i, q,ck)*(vel[q][j])*(liquid_j[q][j])*fe_values.JxW(q);
	  R[i] +=fe_values.shape_value_component(i, q,ck)*(vel[q][j])*(T_j[q][j])*fe_values.JxW(q);
	}

	//sink term latent
	R[i] +=(TCONST/STEno)*(0.5/dt)*fe_values.shape_value_component(i, q, ck)*(3.0*liquid[q]-4.0*liquid_conv[q]+liquid_convconv[q])*fe_values.JxW(q);
	
	//diffusion terms
	for (unsigned int j = 0; j < dim; j++){
	  R[i] +=(1.0/PEno)*fe_values.shape_grad_component(i, q,ck)[j]*T_j[q][j]*fe_values.JxW(q);
	}

	//Laser sinl Term
	//Sink term laser

	Sacado::Fad::DFad<double>  LASER =(emsvty)*DD*(Qlaser/3.1416/rs/rs/th);
	LASER*=std::exp(-(DD/rs/rs)*((qPoint[0]-currentTime)*(qPoint[0]-currentTime)))  ;
	LASER*=std::exp(-(DD/rs/rs)*((2.0*qPoint[2]-problemWidth)*(2.0*qPoint[2]-problemWidth) ))  ;
	LASER*=std::exp(-(DD/th/th)*((qPoint[1]-problemHeight)*(qPoint[1]-problemHeight) ))  ;
	R[i] +=-(1.0)*fe_values.shape_value_component(i, q, ck)*(LASER)*fe_values.JxW(q);
	
      }

      else if (ck==5) {
	Sacado::Fad::DFad<double> FRACTION;
	FRACTION=std::tanh((5.0/(1.0-TSS))*(T[q]-0.5*(1+TSS)));
  ...
  for (unsigned int f=0; f < faces_per_cell; f++) { 
	...
	for (unsigned int q=0; q<n_q_points_face; ++q) {
	  if (ck==4) {  
	    Sacado::Fad::DFad<double>  dTRAD= std::pow(Tface[q]+Tamb,4) - std::pow(Tamb,4);
            R[i] += (BIno/PEno)*fe_face_values.shape_value_component(i, q,ck)*(Tface[q])*fe_face_values.JxW(q);
            R[i] += (TSBOno)*fe_face_values.shape_value_component(i, q,ck)*(dTRAD)*fe_face_values.JxW(q);  
	  }
	}
      }
